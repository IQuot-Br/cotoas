...
# Força o CustomTkinter a usar a renderização nativa padrão do Tkinter
...
class ExecutorApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("CoToAs.net - PC Executor Hub v1.9")
        ...
        self.scrollable_frame = ctk.CTkScrollableFrame(self.sidebar_frame, width=200, height=220)
        ...
        self.btn_load_verified = ctk.CTkButton(self.sidebar_frame, text="ILUSÃO VERIFICADO 💎", fg_color="#104E8B", hover_color="#1E90FF", font=ctk.CTkFont(weight="bold"), command=self.carregar_script_verificado)
        self.btn_load_verified.pack(pady=5, padx=10, fill="x")
        ...
        self.text_box.insert("0.0", "-- Desenvolvido de forma segura por CoToAs.net...\n")
        ...
        self.label_contador = ctk.CTkLabel(self.status_frame, text="Linhas: 2 | Caracteres: 49", font=ctk.CTkFont(size=11), text_color="#888888")
        ...
        self.btn_inject = ctk.CTkButton(self.button_frame, text="INJECT (SAFE)", fg_color="#008B8B", hover_color="#00FFFF", font=ctk.CTkFont(weight="bold"), command=self.acao_injetar_seguro)
    ...
    # LÓGICA E INTERAÇÕES
    # ---------------------------------------------------------------------
    def carregar_script_verificado(self):
        script_verificado = """-- Script Luau Seguro: Ilusão de Selo Verificado Client-Side
...
TextChatService.OnIncomingMessage = function(chatMessage: TextChatMessage)
    ...
    if chatMessage.TextSource and chatMessage.TextSource.UserId == LocalPlayer.UserId then
        local iconeVerificado = "\\u{E000}" 
        propriedades.PrefixText = iconeVerificado .. " " .. chatMessage.PrefixText
    end
    return propriedades
end
print("CoToAs.net: Ilusão visual de verificação ativa no chat.")"""
        self.acao_limpar()
        self.text_box.insert("0.0", script_verificado)
        self.atualizar_contador()
        messagebox.showinfo("CoToAs.net", "Script de Ilusão de Verificado carregado!")

    def acao_injetar_seguro(self):
        ...
        if jogo_encontrado:
            messagebox.showinfo("CoToAs.net", f"Roblox detectado no sistema de forma segura!\n\nPID associado: {pid_encontrado}")
        else:
            messagebox.showwarning("CoToAs.net", f"O processo '{processo_alvo}' não foi detectado.")

    def acao_executar(self):
        script = self.text_box.get("0.0", "end").strip()
        if not script:
            messagebox.showwarning("Aviso", "O editor está vazio.")
            return

        messagebox.showinfo("CoToAs.net", f"Execução simulada do script:\n\n{script[:80]}...")
    ...
    def acao_salvar(self):
        ...
        if nome:
            nome = nome.strip() + ".txt" if not nome.endswith(".txt") else nome.strip()
            with open(os.path.join(self.pasta_scripts, nome), "w", encoding="utf-8") as f:
                f.write(texto)
    ...
    def acao_deletar(self, nome_arquivo):
        confirmar = messagebox.askyesno("Confirmar Exclusão", f"Deseja apagar o script '{nome_arquivo.replace('.txt', '')}'?")
        if confirmar:
    ...
    def acao_carregar(self, nome_arquivo):
        ...
        try:
            ...
            messagebox.showerror("Erro", f"Falha interna ao carregar arquivo:\n{e}")
...
if __name__ == "__main__":
    try:
        ...
        # Se houver qualquer erro escondido impedindo a janela de abrir, ele vai salvar em um arquivo de texto
        with open("erro_inicializacao.txt", "w", encoding="utf-8") as f:
            f.write(f"Erro ao abrir a interface: {e}")
        print(f"Ocorreu um erro ao abrir a janela: {e}")
